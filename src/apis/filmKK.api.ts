import { URL_API_KK } from "@/constant/api.constant";
...
const loadGenres = async () => {
  const response = await axios.get<Genre[]>(`${URL_API_KK}the-loai`);
  return response.data;
};

const loadCountries = async () => {
  const response = await axios.get<Country[]>(`${URL_API_KK}quoc-gia`);
  return response.data;
};

const getFilmsUpdateV3 = async (page: number) => {
  const response = await axios.get<FilmUpdateResponse>(
    `${URL_API_KK}danh-sach/phim-moi-cap-nhat-v3?page=${page}`
  );
  return response.data;
};
const getFilmsUpdateV2 = async (page?: number) => {
  const response = await axios.get<FilmUpdateResponse>(
    `${URL_API_KK}danh-sach/phim-moi-cap-nhat-v2?page=${page || 1}`
  );
  return response.data;
};
  ...
  switch (options.type) {
    case GetFilmsType.COUNTRY:
      ...
      if (options.selectedYear !== "all") {
        url += `&year=${options.selectedYear || new Date().getFullYear()}`;
      }
      break;
